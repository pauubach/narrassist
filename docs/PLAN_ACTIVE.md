...
No existe actualmente un directorio `docs/_archive/` en este repositorio. El histórico relevante sigue en `docs/audits/` y `docs/IMPROVEMENT_PLAN.md`.

## Estado rapido

- S13: cerrado (filtrado por capitulos + comparison summary MVP).
- S14: cerrado en gran parte (Revision Intelligence y track changes).
- S15: completado (version tracking).
- S16A: completado (UX de cuota y tiers, sin pagos).
- S16B: pendiente y condicionado (pagos con backend publico de billing).

## Sprints operativos

## S15 - Version Tracking (cerrado)
...
## S16A - Monetizacion UX (sin pagos, cerrado)
...
- Añadir smoke tests cross-platform de arranque Tauri + sidecar + health check.
- Revalidar accesibilidad/WCAG con pruebas más estrictas y revisión manual.
...
- BK-28: Sprint S15 (completado).
- BK-29: Sprint S16A/S16B (S16A completado, S16B pendiente/condicionado).
...
- Fuente de estado: `docs/audits/2026-03-07_auditoria_integral_estado_actual.md`.
...
El repositorio actual no contiene `docs/_archive/`. Para trazabilidad histórica usa:
