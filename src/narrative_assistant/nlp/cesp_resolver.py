...
class AttributeCandidate:
    ...
    syntactic_evidence: Optional[str] = None
...
class ConflictResolver:
    ...
    def classify_conflict_status(
        ...
        syntactic_candidates = [
            ...
            and c.assignment_source in (
                AssignmentSource.EXPLICIT_SUBJECT,
                AssignmentSource.GENITIVE
            )
        ]
        
        if syntactic_candidates:
            logger.debug(f"[ConflictResolver] CONFIRMED - evidencia sintáctica clara")
        ...
        assigned_entities = {
            ...
            for c in candidates
            if c.assigned_entity_id is not None
        }
        
        if len(assigned_entities) == 1:
            logger.debug(f"[ConflictResolver] UNANIMOUS - todos coinciden")
            return ConflictStatus.UNANIMOUS
        
        logger.debug(f"[ConflictResolver] CONFLICT - desacuerdo entre extractores")
        return ConflictStatus.CONFLICT
    ...
    def resolve(
        ...
        entity_mentions: List[EntityMention],
        text: str
    ) -> Optional[Tuple[str, float, AssignmentSource, List[str]]]:
        """
        ...
        if status == ConflictStatus.CONFIRMED:
            ...
            return (
                ...
                best.base_confidence,
                best.assignment_source,
                notes
            )
            ...
            notes.append(f"Boost por unanimidad: {best.base_confidence:.2f} → {boosted_confidence:.2f}")
            return (
                best.assigned_entity_id,
                boosted_confidence,
            ...
            return self._resolve_conflict_with_arbitration(
                candidates, entity_mentions, text, notes
            )
    ...
    def _resolve_conflict_with_arbitration(
        ...
        if self._llm and self._llm.is_available():
            ...
            if result:
                ...
                notes.append(f"Resuelto por arbitraje LLM")
...
class AttributeDeduplicator:
    ...
    def deduplicate(
        ...
        logger.debug(f"[Deduplicator] {len(attributes)} atributos → {len(attr_groups)} grupos únicos")
        ...
        for key, group in attr_groups.items():
            ...
            if len(group) == 1:
                ...
                sorted_group = sorted(
                    group,
                    key=lambda a: (
                        self._source_priority(a.assignment_source),
                        a.final_confidence
                    ),
                    reverse=True
                )
                ...
                eliminated = [a for a in sorted_group[1:]]
...
class CESPAttributeResolver:
    ...
    def __init__(
        ...
        logger.info(
            f"[CESP] Inicializado, LLM={'disponible' if llm_provider else 'no disponible'}"
        )
    ...
    def resolve(
        ...
        """
        logger.info(f"[CESP] Resolviendo {len(candidates)} candidatos, "
                   f"{len(entity_mentions)} entidades")
        ...
        # =====================================================================
        assigned_candidates = self._ensure_assignments(
            candidates, entity_mentions, text
        )
        
        # =====================================================================
        # PASO 2: Agrupar por atributo y resolver conflictos
        # =====================================================================
        resolved_attributes = self._resolve_by_group(
            assigned_candidates, entity_mentions, text
        )
    ...
    def _assign_by_proximity(
        ...
        same_sentence = [
            ...
            if e.sentence_idx == candidate.sentence_idx
        ]
        ...
        prev_sentence = [
            e for e in entity_mentions
            if e.sentence_idx == candidate.sentence_idx - 1
        ]
