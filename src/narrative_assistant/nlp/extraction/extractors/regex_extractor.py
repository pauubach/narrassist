...
class RegexExtractor(BaseExtractor):
    ...
    HAIR_COLOR_PATTERNS = [
        ...
        (
            r"\b(?P<entity>\w+)\s+ten[ií]a\s+(?:el\s+)?(?:cabello|pelo)\s+(?P<value>negro|rubio|casta[ñn]o|pelirrojo|canoso|blanco|gris)",
            0.9,
        ),
        # "su cabello negro"
        (
            r"[Ss]u\s+(?:cabello|pelo)\s+(?P<value>negro|rubio|casta[ñn]o|pelirrojo|canoso|blanco|gris)",
            0.75,
        ),
        ...
        (
            r"(?:el\s+)?(?:cabello|pelo)\s+(?P<value>negro|rubio|casta[ñn]o|pelirrojo|canoso)\s+de\s+(?P<entity>\w+)",
            0.9,
        ),
        # "cabello largo y negro" -> extrae "negro" (el color después de "y")
        (
            r"(?:cabello|pelo)\s+\w+\s+y\s+(?P<value>negro|rubio|casta[ñn]o|pelirrojo|canoso|blanco|gris)",
            0.85,
        ),
        # "cabello negro y largo" -> extrae "negro" (el color antes de "y")
        (
            r"(?:cabello|pelo)\s+(?P<value>negro|rubio|casta[ñn]o|pelirrojo|canoso|blanco|gris)\s+y\s+\w+",
            0.85,
        ),
        # "Llevaba el cabello corto y rubio"
        (
            r"[Ll]levaba\s+(?:el\s+)?(?:cabello|pelo)\s+\w+\s+y\s+(?P<value>negro|rubio|casta[ñn]o|pelirrojo|canoso)",
            0.85,
        ),
        # "pelo rubio" simple (sin "y")
        (
            r"(?:cabello|pelo)\s+(?P<value>negro|rubio|casta[ñn]o|pelirrojo|canoso|blanco|gris)(?:\s|,|\.)",
            0.80,
        ),
    ]
    ...
    HAIR_TYPE_PATTERNS = [
        ...
        (
            r"llevaba\s+(?:el\s+)?(?:cabello|pelo)\s+(?P<value>recogido|suelto|trenzado|rapado)",
            0.85,
        ),
        # "tenía el pelo largo"
        ...
        (
            r"[Ll]levaba\s+(?:el\s+)?(?:cabello|pelo)\s+(?P<value>largo|corto|rizado|liso|ondulado)",
            0.85,
        ),
    ...
    HEIGHT_PATTERNS = [
        # "era alto/alta"
        (
            r"(?P<entity>\w+)\s+era\s+(?:un\s+hombre\s+|una\s+mujer\s+)?(?P<value>muy\s+)?(?P<height>alt[oa]|baj[oa])",
            0.9,
        ),
        # "un hombre alto"
    ...
    BUILD_PATTERNS = [
        # "era fornido/delgado"
        (
            r"(?P<entity>\w+)\s+era\s+(?:un\s+hombre\s+|una\s+mujer\s+)?(?P<value>fornid[oa]|delgad[oa]|robust[oa]|esbelt[oa])",
            0.9,
        ),
        # "un hombre bajo y fornido"
        (
            r"(?:un|una)\s+(?:hombre|mujer)\s+\w+\s+y\s+(?P<value>fornid[oa]|delgad[oa]|robust[oa])",
            0.85,
        ),
    ...
    PROFESSION_PATTERNS = [
        ...
        (
            r"(?:[Ee]l|[Ll]a)\s+(?P<value>(?:\w+(?:ero|era|ista|or|ora|nte|dor|dora|tor|tora|ogo|oga|ario|aria|ivo|iva|ico|ica|ino|ina|ador|adora)))\s+(?P<entity>[A-ZÁÉÍÓÚÜÑ]\w+)",
            0.90,
        ),
        # "Pedro, un joven carpintero" / "Ana, una gran detective"
        (
            r"(?P<entity>[A-ZÁÉÍÓÚÜÑ]\w+),\s+(?:un[oa]?\s+)?(?:\w+\s+)?(?P<value>\w+(?:ero|era|ista|or|ora|nte|dor|dora|tor|tora|ogo|oga|ario|aria|ivo|iva|ico|ica|ino|ina|ador|adora))\b",
            0.80,
        ),
        ...
        (
            r"(?P<entity>[A-ZÁÉÍÓÚÜÑ]\w+)\s+(?:era|es|fue|será)\s+(?:un[oa]?\s+)?(?P<value>(?:\w+(?:ero|era|ista|or|ora|nte|dor|dora|tor|tora|ogo|oga|ario|aria|ico|ica|ino|ina)))\b",
            0.80,
        ),
        # "la bibliotecaria del pueblo" -> apposition with article
        (
            r"(?:[Ee]l|[Ll]a)\s+(?P<value>(?:\w+(?:ero|era|ista|or|ora|nte|dor|dora|tor|tora|ogo|oga|ario|aria|ivo|iva|ico|ica|ino|ina|ador|adora)))\s+(?:del?\s+\w+)",
            0.75,
        ),
    ]
    ...
    def _compile_patterns(self) -> None:
        ...
        self._eye_patterns = [
            (re.compile(p, re.IGNORECASE), conf) for p, conf in self.EYE_COLOR_PATTERNS
        ]
        self._hair_color_patterns = [
            (re.compile(p, re.IGNORECASE), conf) for p, conf in self.HAIR_COLOR_PATTERNS
        ]
        self._hair_type_patterns = [
            (re.compile(p, re.IGNORECASE), conf) for p, conf in self.HAIR_TYPE_PATTERNS
        ]
        self._height_patterns = [
            (re.compile(p, re.IGNORECASE), conf) for p, conf in self.HEIGHT_PATTERNS
        ]
        self._build_patterns = [
            (re.compile(p, re.IGNORECASE), conf) for p, conf in self.BUILD_PATTERNS
        ]
        self._age_patterns = [(re.compile(p, re.IGNORECASE), conf) for p, conf in self.AGE_PATTERNS]
        self._profession_patterns = [
            (re.compile(p, re.UNICODE), conf) for p, conf in self.PROFESSION_PATTERNS
        ]

        self._metaphor_re = [re.compile(p, re.IGNORECASE) for p in self.METAPHOR_INDICATORS]
        self._negation_re = [re.compile(p, re.IGNORECASE) for p in self.NEGATION_INDICATORS]
        ...
        for pattern, base_confidence in patterns:
            for match in pattern.finditer(text):
                ...
                if not entity or entity.lower() not in entity_names:
                    entity = self._find_nearest_entity(text, match.start(), entity_names_list)
