...
class PacingMetrics:
    ...
    action_verb_ratio: Optional[float] = None
    pacing_score: Optional[float] = None
    balance_deviation: Optional[float] = None
    created_at: Optional[datetime] = None
...
class AnalysisRepository:
    ...
    def complete_run(self, run_id: int, status: str = "completed", error_message: Optional[str] = None) -> None:
    ...
    def get_latest_run(self, project_id: int) -> Optional[AnalysisRun]:
    ...
    def get_executed_phases(self, project_id: int) -> dict[str, bool]:
        ...
        with self.db.transaction() as conn:
            conn.row_factory = lambda c, r: dict(zip([col[0] for col in c.description], r))
            rows = conn.execute(
                "SELECT phase_name, executed FROM analysis_phases WHERE run_id = ?",
                (run.id,)
            ).fetchall()
    ...
    def save_relationship(self, rel: Relationship) -> int:
        ...
        with self.db.transaction() as conn:
            cursor = conn.execute(
                ...
                """,
                (
                    rel.project_id, rel.entity1_id, rel.entity2_id, rel.relation_type,
                    rel.subtype, rel.direction, rel.confidence, rel.chapter_id,
                    rel.start_char, rel.end_char, rel.source_text, rel.detection_method,
                    int(rel.is_inferred)
                )
            )
            return cursor.lastrowid
    ...
    def save_relationships_batch(self, relationships: list[Relationship]) -> int:
        ...
        with self.db.transaction() as conn:
            conn.executemany(
                ...
                [
                    (
                        r.project_id, r.entity1_id, r.entity2_id, r.relation_type,
                        r.subtype, r.direction, r.confidence, r.chapter_id,
                        r.start_char, r.end_char, r.source_text, r.detection_method,
                        int(r.is_inferred)
                    )
    ...
    def save_interaction(self, interaction: Interaction) -> int:
        ...
        with self.db.transaction() as conn:
            cursor = conn.execute(
                ...
                """,
                (
                    interaction.project_id, interaction.entity1_id, interaction.entity2_id,
                    interaction.interaction_type, interaction.tone, interaction.intensity,
                    interaction.chapter_id, interaction.position, interaction.start_char,
                    interaction.end_char, interaction.text_excerpt, int(interaction.is_in_dialogue)
                )
            )
            return cursor.lastrowid
    ...
    def save_interactions_batch(self, interactions: list[Interaction]) -> int:
        ...
        with self.db.transaction() as conn:
            conn.executemany(
                ...
                [
                    (
                        i.project_id, i.entity1_id, i.entity2_id,
                        i.interaction_type, i.tone, i.intensity,
                        i.chapter_id, i.position, i.start_char,
                        i.end_char, i.text_excerpt, int(i.is_in_dialogue)
                    )
    ...
    def get_interactions(self, project_id: int, chapter_id: Optional[int] = None) -> list[Interaction]:
        ...
        with self.db.transaction() as conn:
            ...
            if chapter_id:
                ...
                rows = conn.execute(
                    "SELECT * FROM interactions WHERE project_id = ?",
                    (project_id,)
                ).fetchall()
    ...
    def delete_interactions(self, project_id: int) -> int:
        ...
        with self.db.transaction() as conn:
            cursor = conn.execute(
                "DELETE FROM interactions WHERE project_id = ?",
                (project_id,)
            )
            return cursor.rowcount
    ...
    def save_register_change(self, change: RegisterChange) -> int:
        ...
        with self.db.transaction() as conn:
            cursor = conn.execute(
                ...
                """,
                (
                    change.project_id, change.from_register, change.to_register,
                    change.chapter_id, change.start_char, change.end_char,
                    change.position, change.text_excerpt, change.severity,
                    change.explanation, change.confidence, int(change.is_justified)
                )
            )
            return cursor.lastrowid
    ...
    def save_register_changes_batch(self, changes: list[RegisterChange]) -> int:
        ...
        with self.db.transaction() as conn:
            conn.executemany(
                ...
                [
                    (
                        c.project_id, c.from_register, c.to_register,
                        c.chapter_id, c.start_char, c.end_char,
                        c.position, c.text_excerpt, c.severity,
                        c.explanation, c.confidence, int(c.is_justified)
                    )
                    for c in changes
                ]
            )
    ...
    def get_register_changes(self, project_id: int) -> list[RegisterChange]:
        ...
        with self.db.transaction() as conn:
            conn.row_factory = lambda c, r: dict(zip([col[0] for col in c.description], r))
            rows = conn.execute(
                "SELECT * FROM register_changes WHERE project_id = ?",
                (project_id,)
            ).fetchall()
    ...
    def delete_register_changes(self, project_id: int) -> int:
        ...
        with self.db.transaction() as conn:
            cursor = conn.execute(
                "DELETE FROM register_changes WHERE project_id = ?",
                (project_id,)
            )
            return cursor.rowcount
    ...
    def save_pacing_metrics(self, metrics: PacingMetrics) -> int:
        ...
        with self.db.transaction() as conn:
            cursor = conn.execute(
                ...
                """,
                (
                    metrics.project_id, metrics.chapter_id, metrics.word_count,
                    metrics.sentence_count, metrics.paragraph_count, metrics.dialogue_count,
                    metrics.dialogue_ratio, metrics.avg_sentence_length, metrics.avg_paragraph_length,
                    metrics.lexical_density, metrics.unique_words, metrics.longest_sentence_words,
                    metrics.action_verb_ratio, metrics.pacing_score, metrics.balance_deviation
                )
            )
            return cursor.lastrowid
    ...
    def delete_pacing_metrics(self, project_id: int) -> int:
        ...
        with self.db.transaction() as conn:
            cursor = conn.execute(
                "DELETE FROM pacing_metrics WHERE project_id = ?",
                (project_id,)
            )
            return cursor.rowcount
...
def get_analysis_repository(db: Optional[Database] = None) -> AnalysisRepository:
