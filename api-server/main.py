...
# If running as embedded app (DMG / packaged), attempt a non-blocking
# background post-install to ensure Java, LanguageTool and Ollama are
# installed in Production (first-run behavior). This will not block
# server startup; it logs progress to the early logger.
try:
    if IS_EMBEDDED_RUNTIME:
        def _start_background_post_install():
            try:
                import threading
                import shutil
                from pathlib import Path
                ...
                # If tools dir doesn't exist, we should run installer
                if not tools_dir.exists():
                    need_install = True

                # Check LanguageTool installation
                try:
                    from narrative_assistant.nlp.grammar.languagetool_manager import is_languagetool_installed
                    if not is_languagetool_installed():
                        need_install = True
                except Exception:
                    # If the check fails, be conservative and attempt install
                    need_install = True
                ...
                if need_install:
                    _early_logger.info("Embedded runtime detected: launching background post-install (silent)")
                    # Import the installer script and run in a daemon thread
                    try:
                        from scripts import post_install

                        def _run():
                            try:
                                post_install.run_installation(silent=True, include_ollama=True)
                            except Exception as e:
                                _early_logger.exception("post_install failed: %s", e)

                        t = threading.Thread(target=_run, name="post-install", daemon=True)
                        t.start()
                    except Exception as e:
        ...
        try:
            _start_background_post_install()
        except Exception as e:
            _early_logger.exception("Failed to start background post-install: %s", e)
except Exception:
    # Silently continue if any issue with automatic post-install logic
    _early_logger.exception("Unexpected error in embedded post-install logic")
    pass
