...
def test_crutch_words_100_word_threshold():
    """Verificar que 'o sea' se detecta en textos de 100+ palabras."""
    ...
    # Texto de exactamente 100 palabras con 5 "o sea"
    words = ["palabra"] * 90
    text = "O sea, " + " ".join(words) + " o sea, final o sea, continúa o sea, termina o sea."
    word_count = len(text.split())

    config = CrutchWordsConfig(
        enabled=True,
        min_occurrences=3,  # Bajar para facilitar detección en test
    ...
    # Test mínimo: con 100 palabras no debe retornar vacío por threshold
    print("   OK Test Threshold 100 PASSED (no rechazado por longitud)")


def test_crutch_words_below_threshold():
    """Verificar que textos <100 palabras sí se rechazan."""
    from narrative_assistant.corrections.detectors.crutch_words import CrutchWordsDetector
    ...
    print(f"   - Texto tiene {word_count} palabras (<100)")

    issues = detector.detect(text, chapter_index=1)

    print(f"   - Issues detectados: {len(issues)}")

    assert len(issues) == 0, "Textos <100 palabras deben retornar vacío"
    print("   OK Test Below Threshold PASSED (correctamente rechazado)")


if __name__ == "__main__":
    print("=" * 60)
    print("HOTFIX v0.10.10 - Tests de Validación")
    print("=" * 60)

    try:
        test_pov_chapter_offset()
        test_crutch_words_100_word_threshold()
