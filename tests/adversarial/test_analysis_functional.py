...
class TestAttributesFunctional:
    ...
    PHYSICAL_ATTRIBUTES = [
        ...
        pytest.param(
            "El hombre gordo y bajo cruzó la calle.",
            None,
            ["gordo", "bajo"],
            marks=pytest.mark.xfail(
                reason="Atributos sin entidad nombrada requieren contexto adicional"
            ),
        ),
    ]
...
class TestTimelineFunctional:
    ...
    def test_temporal_marker_detection(self, text, marker, marker_type):
        ...
        pytest.xfail("TimelineBuilder no procesa texto plano - necesita detector de marcadores")
    ...
    def test_flashback_detection(self, text, is_flashback):
        ...
        pytest.xfail("Detector de flashbacks no disponible como componente standalone")
