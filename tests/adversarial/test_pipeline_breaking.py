...
class TestAdversarialNER:
    ...
    def test_ambiguous_names_detection(
        self, ner_extractor, text: str, entity_name: str, should_detect: bool
    ):
        ...
        if should_detect:
            ...
            if not detected:
                pytest.xfail(
                    f"Nombres ambiguos: '{entity_name}' no detectado - punto débil de spaCy con sustantivos como nombres"
                )
    ...
    def test_complex_spanish_names(self, ner_extractor, name: str):
        ...
        if len(person_entities) < 1:
            pytest.xfail(
                f"Nombres españoles complejos: '{name}' no detectado - punto débil con nombres compuestos largos"
            )
    ...
    def test_fantasy_names_detection(self, ner_extractor, text: str, expected_name: str):
        ...
        if not any(expected_name.lower() in t.lower() for t in all_texts):
            pytest.xfail(
                f"Nombre de fantasía '{expected_name}' no detectado (punto débil conocido)"
            )
...
class TestAdversarialAttributes:
    ...
    NEGATED_ATTRIBUTES = [
        ...
        (
            "Dicen que Luis es inteligente, pero yo no lo creo.",
            "Luis",
            "inteligente",
            False,
        ),  # Duda
    ...
    def test_negated_attributes_not_assigned(
        self, text: str, entity: str, attribute: str, should_have: bool
    ):
    ...
    def test_temporal_attribute_changes(self, text: str):
        ...
        pytest.xfail(
            "Atributos temporales: punto débil conocido - no se trackean cambios en el tiempo"
        )
...
class TestAdversarialFusion:
    ...
    def test_similar_names_not_fused(self, name1: str, name2: str):
        ...
        if similarity > 0.7:
            pytest.xfail(
                f"Similaridad alta ({similarity:.2f}) podría causar fusión incorrecta: {name1} ↔ {name2}"
            )
    ...
    def test_nickname_fusion(self, formal: str, nickname: str):
        ...
        if similarity < 0.5:
            pytest.xfail(
                f"Apodo '{nickname}' de '{formal}' tiene baja similaridad ({similarity:.2f}) - requiere diccionario"
            )
...
class TestExtremeTexts:
    ...
    def test_very_long_name(self, ner_extractor):
        ...
        if len(result.value.entities) == 0:
            pytest.xfail(
                "Nombre muy largo no detectado - spaCy tiene dificultades con nombres extremadamente compuestos"
            )
    ...
    def test_mixed_languages(self, ner_extractor):
        ...
        if len(person_entities) < 1:
            pytest.xfail(
                "Texto multilingüe: nombres no detectados en contexto mezclado con inglés/francés/alemán"
            )
