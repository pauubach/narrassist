...
class TestSyntacticAttributionCases:
    ...
    def test_syntactic_case(
        self, case: SyntacticTestCase, dependency_extractor, extraction_context_factory
    ):
        ...
        for entity in case.entities:
            if entity not in case.expected_attributions:
                entity_attrs = [a for a in result.attributes if a.entity_name == entity]
...
class TestGenitiveVsProximity:
    ...
    @pytest.mark.parametrize("text,entities,correct_entity,wrong_entity,attr_type", GENITIVE_CASES)
...
class TestTacitSubjectChains:
    ...
    @pytest.mark.parametrize("text,expected_entity,expected_attrs", TACIT_CHAINS)
